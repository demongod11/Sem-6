...
node* BinomialHeap::persistentInsert(node* H, node* x)
{
    BinomialHeap newBinomialHeap;
    newBinomialHeap.root = copyTree(root);
    newBinomialHeap.root = newBinomialHeap.insert(newBinomialHeap.root, key);
    return newBinomialHeap;
    node* H1 = Initializeheap();
    x->parent = NULL;
    x->child = NULL;
    x->sibling = NULL;
